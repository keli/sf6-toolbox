...
function fmtBlock(ob, rawOb) {
    if (ob == null) return '<span class="safe-zero">?</span>';
    const cls = ob > 0 ? "safe-plus" : ob < 0 ? "safe-minus" : "safe-zero";
    const flipped = rawOb != null && rawOb <= 0 && ob > 0;
    const tag = flipped
        ? ` <span title="${t("tip_block_flip")}" style="color:#f90;font-size:0.85em">↑</span>`
        : "";
    return `<span class="${cls}">${ob >= 0 ? "+" : ""}${ob}</span>${tag}`;
}
...
export function renderCharSelect(state) {
    ...
    sel.innerHTML = state.charList.map((c) => `<option value="${c}">${tChar(c)}</option>`).join("");
...
export function renderKdMoveSelect(state, charName) {
    ...
    const kdMoves = moves.filter((m) => m.knockdowns.length > 0 && !m.name.includes("Drive Impact"));
...
export function renderResults(state, results) {
    ...
    const onBlockFlipped = (r) => r.meaty.onBlock != null && r.meaty.onBlock <= 0 && r.totalBlock > 0;
    if (effectiveOnly) results = results.filter((r) => r.onHitFlipped || onBlockFlipped(r));
    ...
    for (const { kdInfo, kdTypes, sources, rows } of sorted) {
        ...
        const srcHtml = [...moveMap.values()]
            .map(({ move, hitTypes }) => {
                const tags = hitTypes
                    .map((ht) => `<span class="${HIT_TYPE_CLASS[ht] || ""}">[${t(`hit_type_${ht}`) || ht}]</span>`)
                    .join("");
        ...
        const kdTypeLabel = kdTypes.size > 1 ? [...kdTypes].sort().join("/") : kdInfo.kdType;
        const kdAdvText =
            typeof kdInfo.advantageMax === "number" && kdInfo.advantageMax !== kdInfo.advantageMin
        ...
        for (const r of sortedRows) {
            ...
            let flipTag = "";
            if (r.onHitFlipped) {
                const labels = r.unlockedMoves.map((m) => m.cmd).join(", ");
                flipTag = ` <span title="${labels}" style="color:#f90;font-size:0.85em;cursor:default">↑</span>`;
            }

            const totalStr = r.meaty.knockdowns.length
                ? `<span class="kd-adv-cell">${t(r.kdInfo.kdType === "HKD" ? "hkd_label" : "kd_label")}</span>`
                : r.totalAdv != null
                  ? `<span class="total-adv">+${r.totalAdv}</span>${flipTag}`
                  : "?";
            ...
            html += `<td>${totalStr}</td>`;
            html += `<td>${fmtBlock(r.totalBlock, r.meaty.onBlock)}</td>`;
            html += "</tr>";
        }
