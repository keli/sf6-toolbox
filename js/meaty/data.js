...
function getCharDataUrl(state, charName) {
    ...
    return `data/${encodeURIComponent(file)}`;
}

export async function ensureCharData(state, charName) {
    if (!charName) return null;
    if (state.charDataCache.has(charName)) return state.charDataCache.get(charName);
...
export async function loadCharIndex() {
    ...
    if (Array.isArray(payload)) {
        return payload.map((name) => ({ name: String(name), file: `${name}.json` }));
    }
